...
# ----- company setting
                    ...
                    if table_dict:
                        companysetting_dict = cd.get_companysetting(table_dict, user_lang, request)
...
# ----- companyinvoice
                    table_dict = datalist_dict.get('companyinvoice')
                    if table_dict:
...
# ----- schemeitem
                    ...
                    if table_dict:
                        dict_list = d.create_schemeitem_list(filter_dict=table_dict, company=request.user.company)
...
# ----- emplhour (roster page)
                    ...
                    if table_dict and roster_period_dict:
...
# ----- review
                    ...
                    if table_dict and review_period_dict:
...
# - copy schemeitems to template
        ...
        template_schemeitem = m.Schemeitem(
            ...
            onpublicholiday=onpublicholiday,
            istemplate=True,
        )
        if team:
...
# - delete instance
            if is_delete:
                ...
                if instance:
                    ...
                    deleted_ok = m.delete_instance(instance, update_dict, request, this_text)
                    if deleted_ok:
                        # instance will stay after delete, tehrefore must set instance = None
                        instance = None
            else:
# - Create new scheme
                if is_create:
...
# ============ make employee absent
                            ...
                            if shift_option == 'moveto_shift':
                                ...
                                split_dict = make_split_shift(emplhour, upload_dict, comp_timezone, timeformat, user_lang, request)
                                if split_dict:
                        ...
                        if eplh_update_list:
                            for eplh_id in eplh_update_list:
                                if eplh_id != pk_int:
                                    ...
                                    if emplhour:
                                        row_dict = {}
                                        d.create_emplhour_itemdict_from_instance(emplhour, row_dict, comp_timezone)
                                        if dict:
...
# - calculate absent_duration: this is workhours per day, in minutes
            ...
            # set absence to zero on public holidays and weekends TODO is this a correct approach??
...
# 4. get is_publicholiday, is_companyholiday of this date from Calendar
            ...
            is_weekend = (rosterdate_dte.isoweekday() > 5) # saturday = 6, sunday = 7
            is_publicholiday, is_companyholiday = d.get_ispublicholiday_iscompanyholiday(rosterdate_dte, request)
            if not is_weekend and not is_publicholiday:
...
#######################################################
def update_emplhour(emplhour, upload_dict, update_dict, request, comp_timezone, timeformat, user_lang, eplh_update_list):
    ...
    #                    'shift', 'confirmstart', 'confirmend', # shift, confirmstart and confirmend are not model fields
